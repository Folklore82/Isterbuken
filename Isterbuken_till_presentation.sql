...
-- Visar deras ansvarsområden
SELECT besättningNamn, ansvarsområdeTitel FROM besättning INNER JOIN ansvarsområde
  ON besättningId = ansvarsområdeBesättningId;

-- Vilka brottsrubriceringar har vi i databasen
SELECT * FROM brottsrubricering;

-- Vilka besättningsmedlemmar har straffpoäng
SELECT * FROM Straffpoäng_Per_Besättningsmedlem;

-- Vi vill söka på besättningsmedlemmar
CALL Besättning_Sök('kolja');

-- Vi är särkilt intresserade av Störstebosschefen
SELECT besättningNamn, straffregisterDatum, brottrubriceringNamn FROM Störstebosschefens_Straffregister;

-- Kan vi dra några felaktiga? slutssater av deras hobbys?
